...
`bar` files never contain the secrets of a real `Authorization` header (they used to say that the username was a secret, too, but when you use good passwords (i.e. really random and long enough), this is not necessary any more, but it make life much easier to see the username).
