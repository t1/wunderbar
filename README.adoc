...
"Consumer Driven Contracts": ... as a _side effect_, WunderBar also records the expected http interactions into a contract file.
By default this is a Behavior ARchive `bar`, a simple zip file containing properties files for the method, uri, and headers, and json files for the bodiesfootnote:[We currently don't see the necessity to support other content types, open an issue if you _do_ need `xml` or whatever.].
Alternatively, you can configure WunderBar to write an OpenAPI JSON file instead.
...
This is nice, but as a welcome side effect, it _records_ the requests and responses you need for your code to work, and saves them in one or more contract files.
By default this is a single `wunder.bar` file (Behavior ARchive), configured through the `output` attribute of `@WunderBarApiConsumer`.
If you prefer an OpenAPI document, configure your test like this:

[source,java]
----
@WunderBarApiConsumer(output =
    @Output(format = OPENAPI, fileName = "target/openapi.json"))
class ProductsGatewayOpenApiIT { /* ... */ }

@WunderBarApiConsumer(output = {
    @Output(fileName = "target/wunder.bar"),
    @Output(format = OPENAPI, fileName = "target/openapi.json")
})
class ProductsGatewayMultiFormatIT { /* ... */ }
----

Give this file to your API provider, so they can check if their service complies to your requirements.
Use `@WunderBarApiConsumer(output = {})` if you want to disable writing contract files completely.

IMPORTANT: OpenAPI is a lossy interoperability format.
It is suitable only for simple single-interaction cases and other tooling around your API.
WunderBar logs warnings when exporting OpenAPI drops fidelity, and may skip interactions that can't be represented faithfully.
Use BAR when you need precise, full-fidelity provider replay.
...
When you implement an API (i.e. you provide it), you can load a suite of tests that has been stored in a contract file, and run them against your service:
...
There are several ways to load contract files; e.g., you can also load them from maven coordinates.
...
BAR is read with full fidelity; OpenAPI is imported only as a simple, lossy subset intended for straightforward single-interaction cases.
...
And if it doesn't support it, it will show up as soon as the new version of the contract file runs.
...
But the requests and responses in a contract file also contain some more or less random _data_ itself.
...
Contract files never contain the secrets of a real `Authorization` header footnote:[They used to say that the username was a secret, too, but when you use good passwords (i.e. really random and really long), this is not necessary anymore, but it makes life so much easier to see the username.].
