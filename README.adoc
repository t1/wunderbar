...
Low ceremony, code-first https://martinfowler.com/articles/consumerDrivenContracts.html[Consumer Driven Contracts] for Java natives. Supports REST and https://graphql.org[GraphQL].
...
On the API Provider side, the acceptance test has to replace this value with real credentials, e.g. by returning a modified `HttpServerRequest` in a `@BeforeInteraction` method.
